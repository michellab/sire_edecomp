...
#function to differentiate dictionary keys which have numbers or not (residue-residue interactions always have the res number, solvent etc keys do not)
    def hasNumbers(st_in):
        return any(char.isdigit() for char in st_in)

#find min/max values for all interactions with the selected residue.
    max_min_list = []
    ...
    max_bin = 80.0 #int(input("Max bin: "))
    min_bin = -150.0  #int(input("Min bin: "))

    print ("Now using bin range " + str(min_bin) + " to " + str(max_bin))

# make lists which store max and min for each residue. Only add CLJ to this
    residue_list = []
    max_list = []
    min_list = []
    ...
    for key, value in energiesDict.items():
        if key == "interresidues" or key == "intraresidues":
            ...
            x = [float(i) for i in new_joined_list]
            y = np.array(x)
            residue_list.append(int(re.sub('[^0-9]', '', key)))
            max_list.append(max(x))
            min_list.append(min(x))

            (n, bins) = np.histogram(y, bins = 1000, range = (min_bin , max_bin), normed=True)
            n = n / (sum(n))
...
# To count the number of populated and non populated bins, to allow dividion of the total bin addition
    ...
    per_residue_max_min_array = np.vstack((residue_array,min_array))
    per_residue_max_min_array = np.vstack((per_residue_max_min_array, max_array)).T
    min_max_filename = ("min_max_%s" % residue_number)
    np.savetxt(min_max_filename, per_residue_max_min_array)#, fmt=['%d', '%.20f'])
